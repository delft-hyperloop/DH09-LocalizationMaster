...
void setup() {
  ...
  Serial5.begin(115200); // Serial 5 is for receiving from the sensor hub
  Serial4.begin(115200); // Serial4 is for sending to the sensor hub
  Serial.begin(9600);
  EEPROM.write(SLAVE_ID_ADDR, 0xB1); // Initialize EEPROM with size 512 bytes
  SLAVE_ID = EEPROM.read(SLAVE_ID_ADDR);
  if (SLAVE_ID == 0xFF) {
    SLAVE_ID = 0xB1; // Default ID if unset
    EEPROM.write(SLAVE_ID_ADDR, SLAVE_ID);
  }
  configValue = EEPROM.read(EEPROM_ADDR_CONFIG);
}

void loop() {
  if (Serial5.available()>0) {
    if (Serial5.peek() == START_BYTE) {
      timer.begin(sendResponse, 1000); // Start the timer to send response every second
